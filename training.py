  ...
  #          return [None]
    ...
    if isinstance(arrays, list):
        if hasattr(start, '__len__'):
            ...
            for x in arrays:
                if sps.issparse(x):
                    res.append(x[start:stop].toarray())
                else:
                    res.append(x[start:stop])
            return res
    else:
        if hasattr(start, '__len__'):
            if hasattr(start, 'shape'):
                start = start.tolist()
            # return arrays[start]
            if sps.issparse(arrays):
                return arrays[start].toarray()
            else:
                return arrays[start]
        else:
            # return arrays[start:stop]
            if sps.issparse(arrays):
                return arrays[start:stop].toarray()
            else:
                return arrays[start:stop]
